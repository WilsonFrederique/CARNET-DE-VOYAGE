...
const MobileNav = ({closeNav, showNav}:Props) => {
    ...
    return (
        <div>
            ...
            {/* NavLinks  */}
            <div className={`fixed top-0 left-0 h-full w-[85vw] max-w-md bg-gradient-to-br from-rose-900 to-pink-900 backdrop-blur-lg bg-opacity-95 shadow-2xl flex flex-col justify-center transform transition-all duration-300 ease-[cubic-bezier(0.22,1,0.36,1)] ${navOpen} z-[1050]`}>
                {/* Close button - Floating circle with modern design */}
                <button 
                    onClick={closeNav}
                    className="absolute top-6 right-6 p-3 rounded-full bg-white/10 hover:bg-white/20 backdrop-blur-sm transition-all duration-300 group"
                ...
                <nav className="px-8 space-y-8">
                    ...
                    >
                        ...
                        <p className="text-2xl sm:text-3xl font-medium text-white/90 group-hover:text-white transition-all duration-300 py-3 pl-2 transform group-hover:translate-x-2">
                            {link.label}
                        ...
                        {/* Subtle glow effect */}
                        <span className="absolute inset-0 bg-pink-500 opacity-0 group-hover:opacity-10 blur-md transition-opacity duration-300"></span>
                        </div>
                    </Link>
                    ))}
                </nav>
